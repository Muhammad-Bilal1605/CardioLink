...
export const getImagingById = async (req, res) => {
  try {
    console.log('🔍 Imaging request for ID:', req.params.id);
    ...
    console.log('🔍 Imaging found:', imaging ? 'Yes' : 'No');
    if (imaging) {
      console.log('🔍 Imaging ID:', imaging._id);
      console.log('🔍 Imaging patient:', imaging.patientId);
      console.log('🔍 Imaging hospital:', imaging.hospitalId);
    }
    
    if (!imaging) {
      console.log('❌ Imaging not found for ID:', req.params.id);
    ...
    // For patients, check if they are the patient associated with this imaging record
    if (user.firstName && user.lastName) { // This is a patient
      if (imaging.patientId && imaging.patientId._id.toString() !== user._id.toString()) {
        return res.status(403).json({ success: false, error: 'You are not authorized to view this imaging record' });
      }
    }
