...
export const protect = async (req, res, next) => {
  try {
    ...
    try {
      ...
      // Support multiple token shapes: { userId }, { patientId }, { employerId }
      const rawUserId = decoded.userId || decoded.patientId || decoded.employerId;

      if (!rawUserId) {
        console.error('❌ Token payload missing identifier (userId/patientId/employerId)');
        return res.status(401).json({
          success: false,
