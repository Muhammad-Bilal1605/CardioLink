...
class LabsService {
  ...
  Future<List<Map<String, dynamic>>> fetchPatientLabs({
    required String patientId,
    String? authToken,
  }) async {
    final uri = Uri.parse('$_baseUrl/lab-results/patient/$patientId');
  ...
  Future<Map<String, dynamic>> fetchLabById({
    required String labId,
    String? authToken,
  }) async {
    final uri = Uri.parse('$_baseUrl/lab-results/$labId');
    final headers = await _buildHeaders(authToken: authToken);
    final res = await http
        .get(uri, headers: headers)
        .timeout(const Duration(seconds: 30));
    ...
    throw Exception('Failed to load lab result (${res.statusCode})');
  }
}
