...
class _LabListState extends State<LabList> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final list = await _service.fetchPatientLabs(patientId: pid, authToken: token);
      setState(() {
        _items = list;
        _loading = false;
      });
  ...
  void _openReport(Map<String, dynamic> lab) {
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    return _items.where((e) {
      ...
      final hay = [e['testName'], e['testType'], e['facility'], e['doctor'], e['notes']]
          .whereType<String>()
          .map((s) => s.toLowerCase())
          .join(' ');
