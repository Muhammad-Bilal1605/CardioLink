...
import 'procedure_details_sheet.dart';
...
class _ProcedureListState extends State<ProcedureList> {
  final ProceduresService _service = ProceduresService();
  bool _loading = false;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final list = await _service.fetchPatientProcedures(patientId: pid, authToken: token);
      setState(() {
        _items = list;
        _loading = false;
      });
  ...
  void _openDetails(Map<String, dynamic> procedure) {
    ...
    if (_items.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(Icons.medical_information, size: 48, color: Colors.grey),
            const SizedBox(height: 12),
            Text(
              'No procedures found',
              style: GoogleFonts.inter(
