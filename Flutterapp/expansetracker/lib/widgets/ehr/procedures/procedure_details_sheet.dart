...
class ProcedureDetailsSheet extends StatelessWidget {
  final Map<String, dynamic> procedure;

  const ProcedureDetailsSheet({
    super.key,
  ...
  Widget build(BuildContext context) {
    final docs = (procedure['documents'] as List?)?.cast<String>() ?? const <String>[];
    final images = (procedure['images'] as List?)?.cast<String>() ?? const <String>[];
    return SafeArea(
      child: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _kv('Physician', procedure['physician']?.toString()),
              _kv('Indication', procedure['indication']?.toString()),
              const SizedBox(height: 12),
              _section('Findings', procedure['findings']),
              _section('Complications', procedure['complications']),
              _section('Follow-up Plan', procedure['followUpPlan']),
              const SizedBox(height: 16),
              if (docs.isNotEmpty) ...[
                Text('Documents', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: const Color(0xFF2D3748))),
                const SizedBox(height: 8),
                ...docs.map((u) => _fileRow(u)).toList(),
                const SizedBox(height: 12),
              ],
              if (images.isNotEmpty) ...[
                Text('Images', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: const Color(0xFF2D3748))),
                const SizedBox(height: 8),
                ...images.map((u) => _fileRow(u)).toList(),
                const SizedBox(height: 12),
              ],
            ],
          ),
