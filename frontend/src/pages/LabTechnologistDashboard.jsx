...
function LabTechnologistDashboard() {
  ...
  const handleView = (id) => {
    navigate(`/view-lab-result/${id}`);
  };

  const handleDelete = async (id) => {
    const confirmed = window.confirm('Are you sure you want to delete this lab result? This action cannot be undone.');
    if (!confirmed) return;
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.error || data.message || 'Failed to delete lab result');
  ...
  return (
    <DashboardLayout title="Lab Technologist Dashboard" role={user?.role || "lab-technologist"}>
      <div className="max-w-7xl mx-auto">
        <div className="bg-gradient-to-r from-green-600 to-green-800 text-white rounded-md p-5 mb-6 shadow">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold">Lab Technologist Dashboard</h2>
        ...
        <div className="bg-white shadow rounded-md overflow-hidden">
          <div className="px-4 py-4 border-b bg-gray-50 flex items-center justify-between">
            <h3 className="text-base font-semibold text-gray-900">Hospital Lab Results</h3>
            <div className="text-xs text-gray-500">{items.length} records</div>
          </div>
          ...
          {!loading && !error && (
            <>
              ...
              {items.length === 0 && (
                <div className="p-6 text-gray-600">No lab results found.</div>
              )}
              {items.map((item) => (
