...
function HospitalizationFilters({ onFilterChange }) {
  ...
  const statuses = [
    'Admitted', 'Discharged', 'Transferred', 'In Progress'
  ];

  const getActiveFilterCount = () => {
    let count = 0;
    count += filters.admissionType.length;
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-100 px-6 py-4 mb-6">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center">
          ...
          <h3 className="text-lg font-semibold text-green-800">Filter Hospitalizations</h3>
          {filterCount > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-100">
          <div className="flex items-center mb-2">
            ...
            <span className="font-medium text-gray-700">Admission Type</span>
          </div>
          <div className="space-y-1">
            {admissionTypes.map(type => (
              <label key={type} className="flex items-center space-x-2 text-sm">
                <input
                  type="checkbox"
                  checked={filters.admissionType.includes(type)}
                  onChange={() => handleFilterChange('admissionType', type)}
...
export default HospitalizationFilters; 